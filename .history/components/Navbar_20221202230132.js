  ...
  return (
    <>
      <header className='sticky w-full top-0 z-50 '>
        ...
        {/* menu list */}
        <ul ref={ulRef}
        className='hidden bg-black text-white lg:flex flex-col lg:flex-row items-center ml-2 space-y-2 p-4 lg:p-1  w-full lg:space-x-4  lg:items-center
         lg:justify-center lg:mx-auto  text-md font-medium'>

                <li className='lg:w-15 ml-1 text-left lg:text-center w-full'>Home</li>

             
          {
          menuItems.map((link,i)=> {
            return (<li className='w-full lg:w-full text-left lg:text-center ' key={link.tripType}>
              
              <VerticalDropdown as={Fragment}  key={link.tripType} tripType = {link.tripType}    dropdowns= {link.dropdowns}  /> 
            </li>
              
              )
              
            })}
            <li className='w-full lg:w-15 text-left lg:text-center'>About Us</li>

            <li className='w-full lg:w-15 text-left lg:text-center'>Contact </li>
            </ul>
      </header>
