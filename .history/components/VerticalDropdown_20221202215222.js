...
function VerticalDropdown({ tripType, dropdowns }) {
  ...
  return (
    <div>
      ...
      className="relative inline-block w-full lg:w-[250px] ">
        <div>
          <Menu.Button className="flex w-full lg:max-w-3xl lg:justify-center  justify-start
                    ...
                    {({ active }) => (
                      <button
                        className={`${active ? 'bg-green-500 text-white' : 'text-white'
                          } group flex w-full lg:w-[500px] items-center rounded-md px-2 py-2 text-sm`}
                      >
