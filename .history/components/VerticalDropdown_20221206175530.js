...
// second open refers to the state of the non-static menu items.
  ...
  const changeStates = () => {
    ...
    // changing these value on the basis of mobilescreen and mobilecheck----yedi mobile view ma cha bhane- euuta change hune .. arko nahune banaune,.
    setOpen(!open)
    setSecondOpen(!secondOpen)
  }
  return (
    <>
      ...
      className="relative inline-block w-full  ">
        ...
        >
                   ...
                   {({ active }) => (
                     <ul 
                       className={`${active ? 'bg-green-500 text-white' : 'text-white'
                         } group flex w-full list-disc  lg:list-none list-inside items-center border-b-2 border-gray-500   rounded-md px-2 py-2 text-sm`}
                     >
                      <li className='w-full'>{(dropdown.tripSubNames && dropdown.tripSubNames.length )?
                       <HorizontalDropdown mobileCheck={mobileCheck} mobileScreen = {mobileScreen} dropdown = {dropdown} toggleMenu={toggleMenu} togglePlusButton={togglePlusButton} />  : <span className='px-4 text-left'>{dropdown.tripName} </span>} 
                      </li>
                     </ul>
                   )}
                 </Menu.Item>
          ...
          :
          <Menu.Items static   onMouseOver={()=>setOpen(true)} onMouseLeave={()=>setOpen(false)}
          className="lg:absolute w-full lg:-right-24 lg:w-[250px]  lg:top-8 mt-2  origin-top-right  rounded-md bg-black shadow-lg
           ring-1 ring-black ring-opacity-5 focus:outline-none">
           {open &&
           <div>
                 ...
                 <Menu.Item key={i} >
                   {({ active}) => (
                     ...
                     >
                       ...
                       <HorizontalDropdown mobileCheck={mobileCheck} mobileScreen = {mobileScreen} dropdown = {dropdown} toggleMenu={toggleMenu} />  : <span className='px-4 text-left'>{dropdown.tripName} </span>} 
