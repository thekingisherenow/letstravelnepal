...
function Navbar() {
  ...
  return (
    <>
      <header className='lg:sticky w-full top-0 z-50 '>
        ...
        {/* //LOGO */}
          ...
          <div className=' sm:ml-12 md:ml-20 hidden  min-[400px]:inline-block  justify-between items-center lg:border-2  shrink rounded-full shadow-xl'>
            <input type='text' className='flex-grow text-sm text-gray-600 placeholder:truncate mt-6 lg:mt-4 placeholder:text-gray-400 shrink pl-2 bg-transparent outline-none ml-3 '
        ...
        className='hidden bg-black text-white lg:flex flex-col lg:flex-row items-center ml-2 space-y-2 py-4 px-0 lg:py-1  w-full   lg:items-center
         lg:justify-center lg:mx-auto  text-md font-medium'>
          ...
          <Link onClick={()=>setNavMenu(false)}
           href= "/"><li className='lg:w-28 py-2 lg:py-0 text-left lg:text-center w-full  lg:px-0 px-4 mt-2  border-b-2   border-gray-600 lg:border-none'>Home</li></Link>

             
          {
          menuItems.map((link,i)=> {
            return (<li className='w-full lg:w-36 lg:mx-2 px-4 lg:px-0 text-left lg:text-center lg:border-none border-b-2   border-gray-600 items-center' key={link.tripType}>
              
              <VerticalDropdown as={Fragment} mobileCheck={mobileCheck} mobileScreen = {mobileScreen} key={link.tripType} tripType = {link.tripType} toggleMenu={toggleMenu}   dropdowns= {link.dropdowns}  /> 
            </li>
            ...
            <Link  onClick={()=>setNavMenu(false)}
            href= "/aboutus"><li className='w-full py-2 lg:py-0 lg:w-28 text-left lg:text-center lg:my-0  items-center border-b-2   border-gray-600 lg:border-none
              lg:px-0 px-4  my-2'>About Us</li></Link>

            <Link  onClick={()=>setNavMenu(false)}
            href= "/contact"><li className='w-full  lg:py-0 lg:w-28 text-left lg:text-center items-center lg:my-0     border-gray-600 lg:border-none lg:px-0 px-4 my-2'>Contact </li></Link>
            </ul>
      </header>
